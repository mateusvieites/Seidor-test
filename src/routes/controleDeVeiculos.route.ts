...
import { UtilizacaoVeiculoService } from '../services/utilizacaoVeiculo.services';
import { VeiculoService } from '../services/veiculo.services';

export async function utilizacaoVeiculosRoutes(
  fastify: FastifyInstance,
  opts: any
) {
  const uvs = new UtilizacaoVeiculoService();
  ...
  fastify.post(
    ...
    async (
      ...
      const veiculoEmUso = (await uvs.listar()).find(
        (uv) => uv.automovelId === veiculoId && !uv.dataFim
      );
  ...
  fastify.post(
    '/controle-de-veiculos/finalizar/:id',
    {
      schema: {
